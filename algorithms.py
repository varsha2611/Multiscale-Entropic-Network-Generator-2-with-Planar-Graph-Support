...
def check_and_fix_connectivity_planar(G, params):
    new_edges = set()
    ccs = list(nx.connected_components(G))
    if len(ccs) > 1:
        giant_comp = ccs[0]
...
def generate_graph(original, params=None, planar=False):
#main entry point
    if params == None:
        params = {}
    ...
    if planar:
        replica, model_map = revise_graph_planar(G=G, level=0,
        ...
        if exact_interpolation: #occurs only if s1 and s2 are deep-copied from an edge that existed in the original graph
            ...
            while len(new_pairs) < num_target_edges and num_failures < max(10, 3*num_target_edges):
                ...
                if ((u,v) not in new_pairs) and ((v,u) not in new_pairs):
                    new_pairs.add((u,v))
                    if not planarity.is_planar(new_pairs):
                       new_pairs.remove((u,v))
                        #print(u,v)
                else:
                    num_failures += 1
...
def revise_graph_planar(G, level, node_edit_rate, node_growth_rate, edge_edit_rate, edge_growth_rate, params):
#revises a graph at a particular resolution and deeper
    no_more_coarse = len(node_edit_rate) < 2 and len(edge_edit_rate) < 2 and len(node_growth_rate) < 2 and len(edge_growth_rate) < 2
    excess_density = G.number_of_edges() == 0 or nx.density(G) > params.get('coarsening_density_limit', 0.9)
    if no_more_coarse or excess_density:
        ...
        coarse, c_data               = do_coarsen(G, params)
        coarse, model_map            = revise_graph_planar(coarse, level+1, node_edit_rate[1:], node_growth_rate[1:], edge_edit_rate[1:], edge_growth_rate[1:], params)
        c_data_prime                 = copy.deepcopy(c_data)  #we are now going to start playing with it;  wishlist: support injection
        c_data_prime, fine_model_map = interpolate_nodes(coarse, c_data_prime, model_map, params)
        clean_c_data(coarse, c_data_prime)
        G_prime                      = do_uncoarsen_planar(coarse, c_data_prime, model_map, fine_model_map, params)

    #editing of the current level
    tpl_data = compute_topology_data(G, level, params)

    G_prime  = edit_nodes_sequential_planar(G_prime, node_edit_rate, node_growth_rate, tpl_data, params)
    G_prime  = edit_edges_sequential_planar(G_prime, edge_edit_rate, edge_growth_rate, tpl_data, params)
    if 'revise_graph_tester' in params:
        params['revise_graph_tester'](G, G_prime, c_data_prime)
    if params.get('retain_intermediates', False):
        G_prime.model_graph, G_prime.coarser_graph = G, coarse
...
def interpolate_non_edited_nodes_edges(G, c_data, model_map, fine_model_map, params):
    aggregates = c_data['aggregates']
    merged_edges = c_data['merged_edges']
    deep_copying = params.get('deep_copying', True)
    if not deep_copying: assert len(model_map) == 0
...
def interpolate_edited_edges(G, c_data, model_map, fine_model_map, params):
    aggregates = c_data['aggregates']
    merged_edges = c_data['merged_edges']
    deep_copying = params.get('deep_copying', True)
    if not deep_copying: assert len(model_map) == 0
...
def edit_nodes_sequential_planar(G, node_edit_rate, node_growth_rate, tpl_data, params):
    verbose = params.get('verbose', True)
    if verbose:
        print('nn: %d'%G.number_of_nodes())
    try:
    ...
    for new_node in added_node_info:
        ...
        if not planarity.is_planar(G):
            G.remove_edge(new_node, anchor_node)
        else:
            degree_new_node +=1
        num_edges_added += 1
        num_remaining_nbs_to_add -= 1
...
def edit_edges_sequential_planar(G, edge_edit_rate, edge_growth_rate, tpl_data, params):
    #edit edges: first delete, then insert
    verbose = params.get('verbose', True)
    try:
        edit_rate = edge_edit_rate != [] and float(edge_edit_rate[0]) or 0.
    ...
    if enforce_connected:
        new_edges = check_and_fix_connectivity_planar(G, params)
        added_edges_set.update(new_edges)
    ...
    added_edges = 0
    removed_edges = 0
    while added_edges < max(20, 3*target_edges_to_add):
        if num_remaining_edges_to_add <= 0:  #we might overshoot, hence <= 0 not ==
            break
        if npr.rand() > edge_welfare_fraction or len(deprived_nodes) == 0:
            head = random.choice(all_nodes)
        ...
        #sys.stdout.write(',%.3f\n'%nx.clustering(G, head))
            ...
            #added_edges += 1
            #print(head,tail)
            #print('removed edge', removed_edges)
...
def do_uncoarsen_planar(G_coarse, c_data, model_map, fine_model_map, params):
    if isinstance(params.get('algorithm_for_uncoarsening', False), collections.Callable):
        return params['algorithm_for_uncoarsening'](G_coarse, c_data, params)
    print("generating planar network")
    c_data_unedited= interpolate_non_edited_nodes_edges(G_coarse, c_data, model_map, fine_model_map, params)
    clean_c_data(G_coarse, c_data_unedited)

    aggregates    = c_data_unedited['aggregates']
    trapped_edges = c_data_unedited['trapped_edges']
    home_nodes    = c_data_unedited['home_nodes']
    merged_edges  = c_data_unedited['merged_edges']
    ...
    for s1,s2 in G_coarse.edges_iter():
        ...
        for u,v in s1s2:
            ...
            assert v in G_fine
            G_fine.add_edge(u,v)
            if not planarity.is_planar(G_fine.edges()):
                G_fine.remove_edge(u, v)
                removed_edges += 1
    ...
    for seed in trapped_edges:
        for u,v in trapped_edges[seed]:
            ...
            #u or v must have been deleted
    c_data_edted = interpolate_edited_edges(G_coarse, c_data, model_map, fine_model_map, params)
    clean_c_data(G_coarse, c_data_edted)

    aggregates = c_data_edted['aggregates']
    trapped_edges = c_data_edted['trapped_edges']
    home_nodes = c_data_edted['home_nodes']
    merged_edges = c_data_edted['merged_edges']
