...
def average_page_rank(G):
    #for graph with connected components
    length =  nx.pagerank(G)
    sum = 0.0
    count = 0.0
    for key1 in list(length.keys()):
        sum = sum + length[key1]
...
def load_graph(path, params={}, list_types_and_exit=False):
    ...
    loaders = {
            ...
            'dot':nx.read_dot,
            'xdot':nx.read_dot,
            #'graph6_list':nx.read_graph6_list,     # seems no longer supported by networkx -- cls
...
#wishlist: make default
default_metrics += [{'name':'avg page rank',   'weight':1, 'optional':0, 'runningtime': 3, 'function':average_page_rank}]
